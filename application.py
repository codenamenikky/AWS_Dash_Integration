...
def getTagData(inputTag):    
    ...
    endDate = response['Item']['timestamp']
    endDate_dt=dt.datetime.strptime(endDate,"%Y-%m-%d %H:%M:%S")
    unitDelta = dt.timedelta(seconds=10)
    startDate_dt = endDate_dt-200*unitDelta
    startDate = startDate_dt.strftime("%Y-%m-%d %H:%M:%S")    
    response=weatherDatatable.query(
        KeyConditionExpression = Key('tag').eq(inputTag)& Key('timestamp').between(startDate,endDate)
    )
    data = response['Items']
    return data
def getdisplay_df():    
    windSpeeddata = getTagData('Speed')
    windSpeedError = getTagData('SpeedError')
    windDirection = getTagData('Direction')
    data = windSpeeddata+windSpeedError+windDirection
    df = pd.DataFrame(data)
    df=df.pivot(index='timestamp',columns='tag',values='value')
    df.sort_index(ascending=False, inplace=True)
    df['Speed'] = df['Speed'].astype(float)
    df['SpeedError'] = df['SpeedError'].astype(float)
